...
public class JSCommandDispatcher {
    ...
    static class DispatcherException extends RuntimeException {
    ...
    JSCommandDispatcher(ConnectableDeviceWrapper deviceWrapper) {
        ...
        this.module = deviceWrapper.module;
    }
    ...
    public void dispatchCommand(String interfaceName, String methodName, JSCommand command, JSONObject args) {
        ...
        if (method != null) {
            try {
                ...
                if (returnValue instanceof ServiceSubscription) {
                    command.serviceSubscription = (ServiceSubscription<?>) returnValue;
                }
            } catch (InvocationTargetException e) {
                Throwable cause = e.getCause();
                Exception wrappedException = new DispatcherException("Exception calling " + methodName + ": " + Objects.requireNonNull(cause).toString());
    ...
    WebAppSessionWrapper getWebAppSessionWrapper(String objectId) {
        WebAppSessionWrapper wrapper = (WebAppSessionWrapper) module.getObjectWrapper(objectId);
...
//        wrapper.session.connect(command.getResponseListener());
        ...
        wrapper.session.connect(new ResponseListener<Object>() {
            @Override
            public void onError(ServiceCommandError error) {
            ...
            public void onSuccess(Object message) {
                command.success((JSONObject) message);
                wrapper.session.setWebAppSessionListener(new WebAppSessionListener() {
                    ...
                    public void onReceiveMessage(WebAppSession webAppSession, Object message) {
                        try {
                            ...
                            wrapper.sendEvent("message", json);
                        }
                        catch (Exception e) {
                            e.printStackTrace();
                            command.error(e);
    ...
    @CommandMethod
    public void webAppSession_closeLaunchSession(JSCommand command, JSONObject args) throws JSONException {
    ...
    MediaControl getMediaControl(JSCommand command, JSONObject args) throws JSONException {
        if (args.has("objectId")) {
            ...
            MediaControlWrapper wrapper = (MediaControlWrapper) module.getObjectWrapper(objectId);
    ...
    PlaylistControl getPlaylistControl(JSCommand command, JSONObject args) throws JSONException {
        if (args.has("objectId")) {
            String objectId = args.optString("objectId");
            PlaylistControlWrapper wrapper = (PlaylistControlWrapper) module.getObjectWrapper(objectId);
